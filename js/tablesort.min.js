;(function() {
  ...
  var getInnerText = function(el, colIndTarget) {
    var tdInd = 0;
    var colIndTdStart = 0;
    var colInd = 0;

    // determine tdInd and colIndTdStart
    while (colInd < colIndTarget) {
      colInd++;
      if (el[tdInd] !== undefined && colInd == colIndTdStart + el[tdInd].colSpan) {
        tdInd++;
        colIndTdStart = colInd;
      }
    }

    // delta btwn col indice of the start of the td and requested col
    var delta = colInd - colIndTdStart;

    // delta > 0 <=> missing column
    if (delta > 0)
      return el[tdInd].getAttribute('data-sort-'+delta) || '';
    else
      return el[colInd].getAttribute('data-sort') || el[colInd].textContent || el[colInd].innerText || '';
  };
        ...
        for (j = 0; j < totalRows; j++) {
          if (noSorts[j]) {
            ...
            try {
              ...
              if (newRows[j - noSortsSoFar].td == "%hide%")
